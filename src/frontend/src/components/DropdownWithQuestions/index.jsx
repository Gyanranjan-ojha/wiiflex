...
const DropdownWithOptions = ({ jobId }) => {
  ...
  const addQuestions = () => {
    const newQuestions = selectedQuestions.map((question) => ({
      questionType: "written", // 'predefined', may be later determined
  ...
  const cancelSelection = () => {
    // Simply close the dropdown and clear any temporary selections
    setIsOpen(false);
    setSelectedQuestions([]);
  };
  ...
  return (
    <>
      <div
        ref={dropdownRef}
        className="flex flex-col gap-1 self-stretch rounded-lg border-[1px] border-light_blue-300 bg-white-A700"
      >
        <div className="flex items-center gap-2 p-3 cursor-pointer">
          ...
          <Input
            ...
            onChange={handleQuestionInputs}
            onFocus={handleFocus}
            placeholder="Search Questions..."
            className="input-class ml-0 !text-light_blue-700 w-[60%]"
            inputClassName="font-bold update-placeholder-font"
          />
        </div>
        {isOpen &&
